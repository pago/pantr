 ...
 *
 * <strong>Info:</strong> This class has been modified to be usable without symfony.
 *
 * @package    symfony
 * @subpackage util
...
class Finder implements \IteratorAggregate {
	...
	public function exec() {
		...
		for ($i = 0; $i < count($args); $i++) {
			if (is_array($args[$i]) && !method_exists($args[$i][0], $args[$i][1])) {
				throw new FormattedException(sprintf('method "%s" does not exist for object "%s".', $args[$i][1], $args[$i][0]));
			} else if (!is_array($args[$i]) && !is_callable($args[$i])) {
				throw new FormattedException(sprintf('function "%s" does not exist.', $args[$i]));
			}
	...
	/**
	 * Finds all matching files in the given directory as relative
	 * file paths but keeps $dir as their prefix.
	 * 
	 ...
	 *
	 * The return value varies with the number of arguments passed in to the directory.
	 * If there is exactly one argument, then the result will be of type FinderResult.
	 * Otherwise the result will be an array.
	 *
	 * @return FinderResult|array list of files and directories
	 */
	public function in() {
		$files    = array();
		$here_dir = getcwd();
		...
		if($numargs == 1) {
			return new FinderResult(array_unique($files), $arg_list[0]);
		}
...
class FinderResult implements \IteratorAggregate {
	private $files;
	private $in;
	
	public function __construct(array $files, $in) {
		$this->files = $files;
		$this->in = $in;
	}
	...
	public function getRelativeFilePaths() {
		$files = preg_replace('/^'.preg_quote(realpath($this->in), '/').'/', '', $this->files);
...
class sfNumberCompare {
	...
	public function test($number) {
		if (!preg_match('{^([<>]=?)?(.*?)([kmg]i?)?$}i', $this->test, $matches)) {
			throw new FormattedException(sprintf('don\'t understand "%s" as a test.', $this->test));
		}
