...
class FTPDeploy {
	...
	public function upload($files, $dest, $in=null) {
		$this->run(function($ftp) use ($files, $dest, $in) {
			...
			Pake::write("\033[1A");
			Pake::writeAction('ftp', 'upload done');
		});
		return $this;
	}
...
class FTPDeployAction {
	...
	public function upload($files, $in) {
		$this->progressBar = new \Pagosoft\Console\ProgressBar(Pake::out(), count($files));
	...
	public function uploadTree($tree, $stack=array()) {
		foreach($tree as $k => $file) {
			if(is_int($k)) {
				...
				Pake::log()->notice('ftp upload '.$lpath);
			} else {
				array_push($stack, $k);
				$this->ftp->mkdir($k);
				$this->ftp->cd($k);
